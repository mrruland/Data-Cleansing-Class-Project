...
The following descriptions of the variables was created from the features_info.txt 
and features.txt files provided with the data at the URL shown above.  Although 
there may be some small errors in this file (such as the columns labeled with 
...bodybody...it was the document which accompanied the data and the 
*UCI Machine Learning Repository* web site did not have any apparent correction
information.  In any case the discussion here will be consistent with the 
documentation accompanying the data during download.
...
result is the the mean of the mean in a large number of cases is representative
of the actual mean across all values.

The following list contains the variable names in the tidy data set - I have 
retained the original number (**column**) associated with the original data
set as well as the original (uncleaned) name from the original data set 
