...
# directory that the files will be unzipped to 
# change this to point to your directory if the data already exists  
  unzippedfiles="./ProjectFile"

# load the libraries used in the script  
  library(plyr, warn.conflicts=FALSE, quietly=TRUE)
  library(dplyr, warn.conflicts=FALSE, quietly=TRUE)
#  the following libraries loaded with suppressed warnings and 
#  messages to clean terminal output  
...
##  this function is not called if the folder pointed to by the 
##  unzippedfiles variable is found  
  getdata <- function() {
    print("downloading the file")
    fileURL <- 
      paste0("https://d396qusza40orc.cloudfront.net/getdata%2F",
    ...
    z<-file.create(paste0(unzippedfiles,"/CreationDate_",
            as.character(format(Sys.Date(), "%Y%m%d"))))   # creation date  
    if (debug) {print("** files ready")}  ###  for watching the process
  }
################# END of getdata() ###########################

############ Start of cleanvariablenames() ###################  
## cleanvariablenames function is used to reformat the variable names to 
##    conform to recommended naming conventions  
  cleanvariablenames <- function(y) {
    names(y) <- gsub("\\(","",names(y))   ## remove '(')
    names(y) <- gsub("\\)","",names(y))   ## remove ')'
    names(y) <- gsub("\\-","",names(y))   ## remove '-'
    names(y) <- gsub(",","",names(y))     ## remove commas
    ## comment this statement out if you believe in camelCase    
    names(y) <- tolower(names(y))         ## to lower case       
    names(y)
  }
...
# THIS SHOULD NEVER HAPPEN IF THE FILE IS THERE  
  if (!file.exists(unzippedfiles)) {
    if (debug) {print("** data files not found - downloading now")}  ## debug
    dir.create(unzippedfiles)
    getdata()
  }

# I chose to read in the list of txt files in the directories rather than 
# hardcoding the names since I was originally reading in the Inertial 
# Signals data and this made it simpler... it turned out that Inertail
# Signals was not needed for this exercise.
# lvl0 is the top level directory
# lvl1 is the directory containing the base files for the test and train 
#    measurements
# lvl2 is the directory containing the Inertial Signal files for the test and 
#  train measurements  

  if (debug) {
    print(paste0(unzippedfiles,"\\UCI HAR Dataset"),pattern="\\.txt$",
          full.names=T)
  }
  lvl0files <- dir(paste0(unzippedfiles,"\\UCI HAR Dataset"),
                   pattern="\\.txt$",full.names=T)
  testlvl1  <- dir(paste0(unzippedfiles,"\\UCI HAR Dataset/test"),
                   pattern="\\.txt$",full.names=T)
  testlvl2  <- dir(paste0(unzippedfiles,
                       "\\UCI HAR Dataset/test/Inertial Signals"),
                   pattern="\\.txt$",full.names=T)
  trainlvl1 <- dir(paste0(unzippedfiles,"\\UCI HAR Dataset/train"),
                   pattern="\\.txt$",full.names=T)
  trainlvl2 <- dir(paste0(unzippedfiles,
                      "\\UCI HAR Dataset/train/Inertial Signals"),
                   pattern="\\.txt$",full.names=T)
  if (debug) {print("got the directory information")}
  
# read the files from the base directory
  if (debug) {print(lvl0files)}           ## debug
  activitylabels <- read.table(lvl0files[1])
  features <- read.table(lvl0files[2])
  if (debug) {print("got the level 0 data")}  ## debug
...
## measures and features are read in as data.table for improved throughput  
  ...
  if (debug) {print("got the level 1 data")}   ## debug
  
# first step is to combine the columns together for the test data
  test<- cbind(testsubject,testy,testx)
  if (debug) {print(paste0("Dimension of test=",dim(test)))}    ## debug
# next step is to combine the columns together for the train data
  train <- cbind(trainsubject,trainy,trainx)
  if (debug) {print(paste0("Dimension of train=",dim(train)))}  ## debug   
# finally we concatonate the train data to the test data
  mergedset <- rbind(train,test)
  if (debug) {print(paste0("Dimension of mergedset=",dim(mergedset)))} ## debug

## note that at this point we could clean up some of the memory in the event 
## you have a small system to do so uncomment the next line - left in for
##  analysis: "rm(trainx,trainy,testy,testx,trainsubject,testsubject)"  
  
# the Forum discussions led me to ignore the 7 mean values associated with 
# angles these values all contained Mean (capital M) in their names, to include
# those into the list of columns analyzed, simply modify the value grepped for 
# in the next line to "[Mm]ean". Another concern was to eliminate the 
# meanFreg() variables as well and this could have been 
# done by changing the grep function call to include the phrase **,fixed=TRUE**
# immediately preceding the final 2 parentheses in the function call 
#  for example ...[,2]],fixed=TRUE))
  keeplabels <- c(grep("mean|std",features[,2]))  # features we are intested in
# the subject and activity are not in the features file so we add 2 for offset
  keepcolumns <- c(1,2,keeplabels+2)  
  if (debug) {
    print(paste0("# labels retained from features file=",length(keeplabels),
                 ", # of columns=",length(keepcolumns)))}

# get rid of unneeded columns (only keep columns with mean or std 
#  in their name)
  mergedset <- mergedset[,keepcolumns]

# put in the column names
  names(mergedset) <-
    c("subject", "activity", as.character(features[keeplabels,2]))
...
# not sure I completely agree with all the "rules" but I tried to comply 
#  with recommendations   
  names(mergedset) <- cleanvariablenames(mergedset)

# show the "standardized" names if in debug mode to watch the 
...
# build the wide means arrays  
  ...
  ## this is the tidy file which gets uploaded  
  if (viewtables & wide) {View(widegroupmeans)}  
...
##  Here is where we write out the table(s)
  ...
  if (wide) {
    ...
    print(paste0("A wide version of the tidy data set has been written to ",
        "'./widetidydataset'"))
  }
  if (long) {
    write.table(longgroupmeans,file="./longtidydataset.txt",row.names=FALSE)
    print(paste0("A long version of the tidy data set has been written to ",
        "'./longtidydataset'"))
  }  
